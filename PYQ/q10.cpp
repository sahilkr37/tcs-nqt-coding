...
Given an integer array a[], contains the last digit of the registration number of N vehicles traveling on date D
(a positive integer). The task is to calculate the total fine collected by the traffic police department from the vehicles violating the rules.
...
Find will be collected from 5,3 and 7 with an amount of 200 each.Hence, the output = 600.
...
Explanation: Date D=3 means only odd number vehicles with are allowed.
Find will be collected from 2,6 and 8 with an amount of 300 each. Hence, the output = 900
...
int main()
{
    ...
    for (int i = 0; i < size; i++)
    {
        if (isEven == true && arr[i] % 2 != 0)
            fineCount++;
        else if (isEven == false && arr[i] % 2 == 0)
            fineCount++;
    }
    cout << fineCount * fine;
